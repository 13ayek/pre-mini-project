                ...
                <div class="mb-3">
                    ...
                    <select name="employee_id" class="form-control" id="employee_id">
                        ...
                        @foreach ($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('employee_id', $employeeAssignment->employee_id) == $employee->id ? 'selected' : '' }}>
                                {{ $employee->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="service_id" class="form-control" id="service_id">
                        ...
                        @foreach ($services as $service)
                            <option value="{{ $service->id }}" {{ old('service_id', $employeeAssignment->service_id) == $service->id ? 'selected' : '' }}>
                    ...
                    @foreach ($days as $day)
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="days[]" value="{{ $day }}"
                                {{ in_array($day, old('days', $employeeAssignment->days ?? [])) ? 'checked' : '' }}>
